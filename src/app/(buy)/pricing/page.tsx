...
import { useCreateFuyhBillMutation, useGetCoinPricesQuery } from '@/redux/features/payment/paymentApi';
...
const Pricing = () => {
    // Fetch coin prices
    const { data: coinData, isLoading: isCoinsLoading } = useGetCoinPricesQuery({});

    // Default to empty or first item if available (handled in useEffect)
    ...
    const [createFuyhBill, { isLoading: isPaymentLoading }] = useCreateFuyhBillMutation();

    // specific effect to update state when data loads or selection changes
    useEffect(() => {
        if (coinData?.data && coinData.data.length > 0) {
            // If nothing selected, select the first one
            if (!coin) {
                const firstPkg = coinData.data[0];
                setCoin(firstPkg.coin.toString());
                setPrice(firstPkg.price.toString());
                setSelectedPackageId(firstPkg._id);
            }
        }
    }, [coinData, coin]);
    ...
    const handleCoinChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        // Find the package corresponding to this coin amount
        const pkg = coinData?.data?.find((p: any) => p.coin.toString() === selectedCoin);
        if (pkg) {
    ...
    const handleConfirm = async () => {
        if (!price) {
            toast.error("Please select a valid coin package.");
            return;
        }

        try {
            const result = await createFuyhBill({
                amount: parseInt(price), // Sending price as amount
            }).unwrap();
            ...
            <main>
                <div className='flex justify-center text-center'>
                    <div className="w-full max-w-7/12 flex flex-col gap-[48px]">
                        ...
                        {!isCoinsLoading && (!coinData?.data || coinData.data.length === 0) && (
                        ...
                        {!isCoinsLoading && coinData?.data && coinData.data.length > 0 && (
                            <form onSubmit={handlePayClick}>
                                <div className="flex flex-col gap-[64px]">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-[60px]">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="relative">
                                                ...
                                                <select
                                                    className='coin text-white w-full border border-[#37404E] rounded-lg px-3 py-4 bg-transparent outline-none appearance-none'
                                                    name="coin"
                                                    value={coin}
                                                    onChange={handleCoinChange}
                                                    disabled={isCoinsLoading}
                                                >
                                                    {isCoinsLoading && <option className='bg-black'>Loading...</option>}
                                                    {coinData?.data?.map((pkg: any) => (
                                                        <option key={pkg._id} className='bg-black' value={pkg.coin}>
                                                            {pkg.coin}
                                                        </option>
                                                    ))}
                                                </select>
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <input
                                                ...
                                                value={price ? `$${price}` : ''}
                                                readOnly
                                                className="w-full price border border-[#37404E] rounded-lg px-3 py-4 text-[#E6E6E6] font-['inter'] text-xl font-medium bg-transparent focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-not-allowed opacity-80"
                                            />
                                        </div>
                                    </div>
                                    ...
                                    <button
                                        type="submit"
                                        disabled={isCoinsLoading || !coin}
                                        className="w-full cursor-pointer flex justify-center items-center text-[#E6E6E6] border-[1px] border-blue-500/50 py-3 rounded-lg font-['inter'] text-xl font-medium transition hover:bg-blue-500/10 disabled:opacity-50"
                                    >
                    ...
                    <div className="p-[1px] bg-gradient-to-b from-[#0685F1] to-[#AAD8FF] border">
                        <div className="relative bg-[#1A1A1A]/100 rounded-xl p-8 max-w-md text-center z-10">
                            ...
                            <h3 className="text-[#D8D8D8] font-['inter'] font-medium lg:text-2xl/normal mb-4">
                                Are you sure you want to spend <span className="text-blue-400">${price}</span> to purchase this plan?
                            </h3>
                            <div className="flex flex-row-reverse justify-center gap-4 mt-6">
                                ...
                                >
                                    {isPaymentLoading ? <LoadingSpinner size={20} /> : "Yes"}
                                </button>
                                <button
